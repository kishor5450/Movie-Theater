...
function Theaterseats(app) {
    ...
    app.post('/theater_seats', (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (theater_id === undefined || t_rows === undefined || t_columns === undefined) {
            return res.status(400).json({ message: 'Missing required fields: theater_id, t_rows, and t_columns.' });
        }
        const query = 'INSERT INTO theater_seats (theater_id, t_rows, t_columns) VALUES (?, ?, ?)';
        try {
            const [results] = yield promisePool.query(query, [theater_id, t_rows, t_columns]);
            ...
            res.status(500).json({ message: 'Error inserting theater seating configuration' });
        }
    }));
    ...
    app.get('/theater_seats', (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            res.status(500).json({ message: 'Error fetching theater seating configurations' });
        }
    }));
    ...
    app.get('/theater_seats/:id', (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (!Array.isArray(results) || results.length === 0) {
                return res.status(404).json({ message: 'Theater seating configuration not found' });
            }
            res.json(results[0]);
            ...
            res.status(500).json({ message: 'Error fetching theater seating configuration' });
        }
    }));
    ...
    app.put('/theater_seats/:id', (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (theater_id === undefined && t_rows === undefined && t_columns === undefined) {
            return res.status(400).json({ message: 'At least one field (theater_id, t_rows, or t_columns) is required for update.' });
        }
        ...
        try {
            ...
            res.status(200).json({ message: 'Theater seating configuration updated successfully' });
            console.log('Updated theater seating configuration:', results);
        }
        catch (err) {
            console.error('Error updating theater seating configuration:', err);
            res.status(500).json({ message: 'Error updating theater seating configuration' });
        }
    }));
    ...
    app.delete('/theater_seats/:id', (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            res.status(200).json({ message: 'Theater seating configuration deleted successfully' });
            console.log('Deleted theater seating configuration:', results);
        }
        catch (err) {
            console.error('Error deleting theater seating configuration:', err);
            res.status(500).json({ message: 'Error deleting theater seating configuration' });
        }
    }));
}
